...
@PhpCsFixer:risky
  Using the :doc:`@PhpCsFixer:risky <./../../ruleSets/PhpCsFixerRisky>` rule set will enable the ``native_function_invocation`` rule with the config below:

  ``['include' => ['@compiler_optimized'], 'scope' => 'namespaced', 'strict' => true]``

@Symfony:risky
  Using the :doc:`@Symfony:risky <./../../ruleSets/SymfonyRisky>` rule set will enable the ``native_function_invocation`` rule with the config below:
